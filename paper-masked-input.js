...
class PaperMaskedInput extends PolymerElement {
  static get template() {
    ...
    <paper-input label="[[label]]"
      type="[[_computeInputType(visible)]]" value="{{value}}" error-message="[[errorMessage]]"
      invalid="{{invalid}}" allowed-pattern="[[allowedPattern]]" validator="[[validator]]"
      pattern\$="[[pattern]]" autocomplete\$="[[autocomplete]]" required\$="[[required]]"
      autofocus\$="[[autofocus]]" inputmode\$="[[inputmode]]" minlength\$="[[minlength]]"
      maxlength\$="[[maxlength]]" name\$="[[name]]" placeholder\$="[[placeholder]]"
      readonly\$="[[readonly]]" size\$="[[size]]" autocapitalize\$="[[autocapitalize]]"
      autocorrect\$="[[autocorrect]]" tabindex="0" disabled\$="[[disabled]]"
      no-label-float="[[noLabelFloat]]" always-float-label="[[alwaysFloatLabel]]"
      auto-validate\$="[[autoValidate]]" id="input">
      <slot name="prefix" slot="prefix"></slot>
      <paper-icon-button id="toggleButton" slot="suffix" on-click="toggle"
        class="action-icon" icon="[[_computeToggleIcon(visible)]]"
        alt="Toggle icon" disabled\$="[[disabled]]"
        title\$="[[_computeToggleLabel(visible)]]"></paper-icon-button>
